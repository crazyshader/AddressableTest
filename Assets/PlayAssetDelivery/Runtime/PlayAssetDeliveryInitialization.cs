        ...
        void DownloadCustomAssetPacksData()
        {
            ...
            www.SendWebRequest().completed += (op) =>
            {
                ...
                if (webrequest.isNetworkError)
                    CompleteOverride($"Could not load '{CustomAssetPackUtility.kCustomAssetPackDataFilename}' : {webrequest.error}.");
                else
                {
                    InitializeBundleToAssetPackMap(webrequest.downloadHandler.text);
